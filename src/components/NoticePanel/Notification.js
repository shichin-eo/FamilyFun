...
const Notification = ({ type, notification }) => {
  const [title, setTitle] = useState(notification["notification_title"]);
  const [body, setBody] = useState(notification["notification_body"]);
  ...
  const HEADER_NOTIFICATION =
    type === "push" ? `Push-уведомление` : `Событие недели`;
  const ICON_NOTIFICATION = type === "push" ? "../assets/img/clock.png" : "";
  return (
    <>
      <div className={`notification ${type}`}>
        <div className={`notification_component `}>
          <div className="notification_component_header">
            <div className={`notification_component_header_title ${type}`}>
          ...
          <div className={`notification_component_body ${type}`}>
            <textarea value={body} onChange={bodyHandler}>
              {" "}
            </textarea>
          </div>
        </div>
        <div className="notification_btns">
          <FontAwesomeIcon
            className="notification_btns_edit"
            icon={faEdit}
            onClick={() => {}}
          />
          <FontAwesomeIcon
            className="notification_btns_accept"
            icon={faCheck}
            onClick={() => {}}
          />
          <FontAwesomeIcon
            className="notification_btns_cancel"
            icon={faTimes}
            onClick={() => {}}
          />
        </div>
      ...
      {/* <div className={`notification-container ${state.type}`}>
        <div className="notification-component">
          ...
          <div className={`notification-body ${state.type}`}>
            <textarea
              value={state.body}
              onChange={(e) =>
                handleChangeNotice(e.target.value, "push", "body")
              }
            >
              {" "}
            </textarea>
        ...
        <div className="notification-control-btns">
          ...
          <div className="notify-accept">
            ...
            >
              <path
                d="M7.75781 14.6875C8.14844 15.0781 8.8125 15.0781 9.20312 14.6875L20.6875 3.20312C21.0781 2.8125 21.0781 2.14844 20.6875 1.75781L19.2812 0.351562C18.8906 -0.0390625 18.2656 -0.0390625 17.875 0.351562L8.5 9.72656L4.08594 5.35156C3.69531 4.96094 3.07031 4.96094 2.67969 5.35156L1.27344 6.75781C0.882812 7.14844 0.882812 7.8125 1.27344 8.20312L7.75781 14.6875Z"
                fill="white"
              />
            </svg>
          </div>
          <div className="notify-remove">
            <svg
              viewBox="0 0 14 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
